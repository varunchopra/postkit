...
CREATE OR REPLACE FUNCTION lease.set_tenant(p_tenant_id text)
RETURNS void AS $$
BEGIN
    PERFORM lease._validate_namespace(p_tenant_id);
    PERFORM set_config('lease.tenant_id', p_tenant_id, true);
END;
$$ LANGUAGE plpgsql SET search_path = lease, pg_temp;


-- @function lease.clear_tenant
-- @brief Clear the tenant context.
-- Call before returning connections to pool.
CREATE OR REPLACE FUNCTION lease.clear_tenant()
RETURNS void AS $$
BEGIN
    PERFORM set_config('lease.tenant_id', '', true);
END;
$$ LANGUAGE plpgsql SET search_path = lease, pg_temp;


-- @function lease.set_actor
-- @brief Set actor context for the event log.
...
-- Actor context is captured on leases and lease.events rows.
CREATE OR REPLACE FUNCTION lease.set_actor(
...
RETURNS void AS $$
BEGIN
    PERFORM set_config('lease.actor_id', COALESCE(p_actor_id, ''), true);
    PERFORM set_config('lease.request_id', COALESCE(p_request_id, ''), true);
    PERFORM set_config('lease.on_behalf_of', COALESCE(p_on_behalf_of, ''), true);
    PERFORM set_config('lease.reason', COALESCE(p_reason, ''), true);
END;
$$ LANGUAGE plpgsql SET search_path = lease, pg_temp;


-- @function lease.clear_actor
-- @brief Clear actor context.
-- Call before returning connections to pool.
CREATE OR REPLACE FUNCTION lease.clear_actor()
RETURNS void AS $$
BEGIN
    PERFORM set_config('lease.actor_id', '', true);
    PERFORM set_config('lease.request_id', '', true);
    PERFORM set_config('lease.on_behalf_of', '', true);
    PERFORM set_config('lease.reason', '', true);
END;
$$ LANGUAGE plpgsql SET search_path = lease, pg_temp;


-- @function lease._get_actor_context
...
CREATE OR REPLACE FUNCTION lease._get_actor_context()
RETURNS TABLE(
    actor_id text,
    request_id text,
    on_behalf_of text,
...
BEGIN
    RETURN QUERY SELECT
        nullif(current_setting('lease.actor_id', true), ''),
        nullif(current_setting('lease.request_id', true), ''),
        nullif(current_setting('lease.on_behalf_of', true), ''),
        nullif(current_setting('lease.reason', true), '');
END;
$$ LANGUAGE plpgsql STABLE PARALLEL SAFE SECURITY INVOKER SET search_path = lease, pg_temp;


-- @function lease._rls_bypassed
...
CREATE OR REPLACE FUNCTION lease._rls_bypassed()
RETURNS boolean AS $$
    SELECT rolsuper OR rolbypassrls FROM pg_roles WHERE rolname = current_user;
$$ LANGUAGE sql STABLE SECURITY INVOKER SET search_path = lease, pg_temp;


-- @function lease.assert_rls_active
-- @brief Raise unless row-level security applies to the current role.
-- @example SELECT lease.assert_rls_active();
...
CREATE OR REPLACE FUNCTION lease.assert_rls_active()
RETURNS void AS $$
BEGIN
    IF lease._rls_bypassed() THEN
        RAISE EXCEPTION 'Role % bypasses row-level security', current_user
            USING ERRCODE = 'object_not_in_prerequisite_state',
                  HINT = 'postkit:lease:BIZ_RLS_NOT_ACTIVE';
    END IF;
END;
