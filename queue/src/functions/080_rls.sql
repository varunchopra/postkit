...
CREATE OR REPLACE FUNCTION queue._rls_bypassed()
RETURNS boolean AS $$
    SELECT rolsuper OR rolbypassrls FROM pg_roles WHERE rolname = current_user;
$$ LANGUAGE sql STABLE SECURITY INVOKER SET search_path = queue, pg_temp;


-- @function queue.assert_rls_active
-- @brief Raise unless row-level security applies to the current role.
-- @example SELECT queue.assert_rls_active();
...
CREATE OR REPLACE FUNCTION queue.assert_rls_active()
RETURNS void AS $$
BEGIN
    IF queue._rls_bypassed() THEN
        RAISE EXCEPTION 'Role % bypasses row-level security', current_user
            USING ERRCODE = 'object_not_in_prerequisite_state',
                  HINT = 'postkit:queue:BIZ_RLS_NOT_ACTIVE';
    END IF;
END;
