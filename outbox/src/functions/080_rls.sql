...
CREATE OR REPLACE FUNCTION outbox.assert_rls_active()
RETURNS void AS $$
BEGIN
    IF outbox._rls_bypassed() THEN
        RAISE EXCEPTION 'Role % bypasses row-level security', current_user
            USING ERRCODE = 'object_not_in_prerequisite_state',
                  HINT = 'postkit:outbox:BIZ_RLS_NOT_ACTIVE';
    END IF;
END;
