...
| [`ack_batch`](sdk.md#ack_batch) | Acknowledge multiple jobs atomically. |
...
| [`fail`](sdk.md#fail) | Move a job directly to the dead-letter queue without retrying. |
...
| [`nack`](sdk.md#nack) | Schedule another attempt, or dead-letter the job when none remain. |
| [`pause_schedule`](sdk.md#pause_schedule) | Pause an active schedule. |
| [`pull`](sdk.md#pull) | Pull one job from a queue. |
| [`pull_any`](sdk.md#pull_any) | Pull one job from the first available queue in priority order. |
...
| [`release`](sdk.md#release) | Return a running job to pending immediately, without backoff. |
...
| [`queue.ack_batch`](sql.md#queueack_batch) | Acknowledge multiple jobs atomically. |
| [`queue.cancel`](sql.md#queuecancel) | Cancel a pending job by deleting it. |
| [`queue.fail`](sql.md#queuefail) | Move a running job directly to the dead-letter queue. |
| [`queue.nack`](sql.md#queuenack) | Schedule another attempt with backoff, or dead-letter the job when none remain. |
| [`queue.purge_queue`](sql.md#queuepurge_queue) | Delete all pending jobs from a queue. |
| [`queue.release`](sql.md#queuerelease) | Return a running job to the queue immediately. |
...
| [`queue.pull`](sql.md#queuepull) | Pull one available job and return its fence token. |
| [`queue.pull_any`](sql.md#queuepull_any) | Pull one job from the first queue with available work. |
| [`queue.pull_batch`](sql.md#queuepull_batch) | Pull up to a limit of available jobs in one operation. |
