...
HEADER
    ...
    for module in authn authz config lease meter outbox queue; do
...
case "$MODULE" in
    all)
        build_all
        ;;
    authn|authz|config|lease|meter|outbox|queue)
        build_module "$MODULE"
        ;;
    *)
        echo "Usage: $0 [all|authn|authz|config|lease|meter|outbox|queue]" >&2
        exit 1
        ;;
esac
